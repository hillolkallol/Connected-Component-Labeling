...
public class CCL6 {
    ...
    private void secondPass() {
        ...
        for(int i=0; i<width; i++) {
            for(int j=0; j<height; j++) {
                if(labeling[i][j]!=0) {
                    labeling[i][j] = minimum(labeling[i][j]);
                }
            }
        }
    ...
    private void output() throws IOException {
        ...
        for(int value=1; value<=newLabel; value++){
            ...
            // getting new images
            if(yes)
                ImageIO.write(OutputImage, "png", new File("four" + value + "." + "png"));
        }
    }
    
    private boolean isExist(int x, int i0) {
        //System.out.println(x);
        //System.out.println(i0 + "break");
        boolean mark = false;
        Collection<Integer> col = equLabels.get(x);
        Integer[] values = col.toArray (new Integer[col.size()]);
        for(int y=0; y<col.size(); y++){
    ...
    public static void main(String[] args) throws IOException {
    	...
    	CCL6 ccl6 = new CCL6();
    	try {
            input = ImageIO.read(new File(CCL6.class.getResource("two.png").toURI()));
